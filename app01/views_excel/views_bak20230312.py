...
#def ximprimirFolha(request):
#def xtabelaRegenteSuporteMatriz(request):
#def xtabelaMatriz
#def xfun_nova_listaEventos
#def xlista_Soma_Eventos
#def xdictfetchall
#def xlista_Regente_Suporte
#def xfun_lista_servidores_com_pos
#def xfun_lista_participacao
#def xfun_lista_impressao_a
#def xcoletaDados
#def xfun_dictAmpliacaoCh
#def xtabelaRegenteSuporte
#def xsomaEventos
#def xlista_SomaEventos
#def xfun_Auxiliar_tabelaMatriz
...
def lista_Regente_Suporte(id_municipio,anomes,tabela):
    ...
    qFolha=fun_lista_participacao(id_municipio,anomes)
    ...
    if eventos_ampliacao>0:
        ...
        tupla_eventos_ampliacao=None
    listao=fun_lista_impressao_a(qFolha,tupla_eventos_ampliacao,id_municipio,anomes)
...
def fun_lista_participacao(id_municipio,anomes):
    ...
    and salario>0 group by fm.posgraduacao,fu.id_funcao,salario_100H_int,carga_horaria,fm.suporte,fm.vinculo order by fm.id_funcao,salario_100H_int",[id_municipio,anomes])
...
def fun_lista_impressao_a(qLista,tupla_ampliacaoch,id_municipio,anomes):

    if tupla_ampliacaoch:
        dictAmpliacaoCh=fun_dictAmpliacaoCh(id_municipio,anomes,tupla_ampliacaoch)
    else:
        dictAmpliacaoCh=None
    ...
    for qy in qLista:
        ...
        if [qy['posgraduacao'],qy['suporte'],qy['vinculo'],qy['id_funcao'],qy['salario_100H_int']] not in lista_controle:
            lista_controle.append([qy['posgraduacao'],qy['suporte'],qy['vinculo'],qy['id_funcao'],qy['salario_100H_int']])
            funcao=dictFuncoes.get(qy['id_funcao'],'')
            if qy['id_funcao'] in lista_funcao_prof:
                tipo_funcao=21
            else:
                tipo_funcao=11
            lista=coletaDados(tipo_funcao,funcao,qy['id_funcao'],qy['salario_100H_int'],qy['suporte'],qy['vinculo'],qy['salario_100H'],qLista,dictAmpliacaoCh,qy['posgraduacao'])
            listao.append(lista)
...
def coletaDados(tipo_funcao,funcao,id_funcao,salario_100H_int,suporte,vinculo,salario_100H,queryLista,dictAmpliacao,posgraduacao):
    ...
    for qq in queryLista:
        ...
        if qq['posgraduacao']==posgraduacao and qq['suporte']==suporte and qq['vinculo']==vinculo and qq['id_funcao']==id_funcao and qq['salario_100H_int']==salario_100H_int:
            ...
            if int(qq['carga_horaria'])==100:
                ...
                salario100=qq['salario_100H']
            if dictAmpliacao:
                string=posgraduacao+suporte+vinculo+str(qq['id_funcao'])+str(qq['salario_100H_int']) 
...
def fun_dictAmpliacaoCh(id_municipio,anomes,tupla_eventos_ampliacao):
    cursor = connection.cursor()
    ...
    group by fm.posgraduacao,fm.suporte,fm.vinculo,fm.id_funcao,CAST(fm.salario_100H AS DECIMAL(0)) order by fm.posgraduacao,fm.suporte",[id_municipio,anomes,tupla_eventos_ampliacao])
    query = dictfetchall(cursor)

    lisSalario=[]
    lisParticipacao=[]
    for qp in query:
        chave = qp['posgraduacao']+qp['suporte']+qp['vinculo']+str(qp['id_funcao'])+str(qp['salario_100H_int'])
...
def tabelaRegenteSuporte(id_municipio,anomes,tabela):
    if 1==1:
        ...
        for kl in range(2):
            ...
            writer.writerow(['',funcoes.formatMilhar(s1),funcoes.formatMilhar(s2),funcoes.formatMilhar(s3),funcoes.formatMilhar(s4),funcoes.formatMilhar(s5),'',funcoes.formatMilhar(s6)])
            ...
            if kl==0 or kl==1:
                ...
                writer.writerow(['Sub Total',funcoes.formatMilhar(s1)])
                ...
                nlinha_termino+=1
                form1="=SOMA(B"+str(nlinha_inicio)+":B"+str(nlinha_termino)+")"
                writer.writerow([total_soma_evento[kl],funcoes.formatMilhar(s1)])
